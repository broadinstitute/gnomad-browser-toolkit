...
import { Grid, Typography, ThemeProvider, createMuiTheme, CssBaseline, Container } from "@material-ui/core"
...
interface Props<Item extends TestItem> extends Omit<ClassificationViewerProps<Item>, "setFilterPredicates"> {
  items: Item[]
}
function TestWrapper<Item extends TestItem> ({items, classifications}: Props<Item>) {
  ...
  const filteredElems = filtered.map((elem, index) => (
    <Typography align="center" key={index}>{elem.name}</Typography>
  ))
  return (
    <>
    <CssBaseline/>
    <ThemeProvider theme={theme}>
      <Grid container={true} spacing={1}>
        ...
        <Grid item={true} xs={6}>
          ...
          <Container data-cy={selectedItemsContainerCypressDataAttr}>
            {filteredElems}
          </Container>
        </Grid>
      </Grid>
